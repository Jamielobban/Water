        ...
        Pass
        {
            ...
            half4 Frag(Varyings input, FRONT_FACE_TYPE face : FRONT_FACE_SEMANTIC) : SV_Target
            {
                ...
                float foam = 0;
                    ...
                    // Reject surfaces above the waterline, while retaining a wide shallow-ground band.
                    float groundOnly=smoothstep(0.015,0.08,submergedGround)*exp(-submergedGround*1.8);
                    float shore = saturate(1-submergedGround/max(_ShoreFoamWidth,0.01))*groundOnly;
                    ...
                    if (_StylizedSteps < 2)
                    {
                        ...
                        float shoreBand=(1-smoothstep(_ShoreFoamWidth*0.35,_ShoreFoamWidth,shoreDepth))
                            *smoothstep(0.01,0.12,submergedGround);
                    ...
                    foam=max(max(shoreFoam,crestFoam),ripple.z*_InteractionFoamStrength*pattern);
